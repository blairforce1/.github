...
# Bot authors: no Checks section or provenance, but title, Change line and
# class label as for anyone. RENOVATE is the body its prBodyNotes produce.
RENOVATE=$'This PR contains the following updates:\n\n| Package | Change |\n|---|---|\n| lefthook | 2.1.14 -> 2.1.15 |\n\nChange: none\n\nRecord: none\n\n---\n\nThis PR was generated by Mend Renovate.'
BUMP=$'build(deps): update dependency lefthook to v2.1.15'
LABELS='["class:infra"]'
run "bot with the label and the body notes"        0 "Opened by renovate[bot]" "$BUMP" "$RENOVATE" 'renovate[bot]' "$BUMP"
LABELS='[]'
run "bot without the class label"                  1 "title=Class label::No class:* label" "$BUMP" "$RENOVATE" 'renovate[bot]' "$BUMP"
LABELS='["class:infra"]'
run "bot without the Change line"                  1 "No 'Change:' line" "$BUMP" "$(printf '%s' "$RENOVATE" | grep -v '^Change:')" 'renovate[bot]' "$BUMP"
LABELS='["class:infra"]'
run "bot with a non-conventional title"            1 "not a conventional commit" "Bump lodash from 1 to 2" "$RENOVATE" 'dependabot[bot]'
LABELS='["class:infra"]'
run "[bot] only as a suffix of the login"          1 "No '## Checks' section" "$BUMP" "$RENOVATE" 'renovate[bot]-impostor' "$BUMP"
